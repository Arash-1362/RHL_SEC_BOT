...
if(entitie !== undefined){
    ...
    return new Promise((resolve, reject) => { // Resualt of safe or unsafe URL submitted by user will be shown by calling Adabtive cards
        ...
        function onEnd() {
            ...
            if (found) {
                step.context.sendActivity({text: "Password vulnarable!  Found matches :  " + found.count + " !! "});
            } else {
...
async confirmStep(step){ //Double check the value yser have entered 
    ...
    await step.context.sendActivity(msg);
    return await step.prompt(CONFIRM_PROMPT, 'Are you sure the password is correct and you want to go ahead?', ['yes', 'no']);
}
